...
*running a multivariate regression analysis;
proc glm data = finaloutput;
  class final_dust (ref= first);
  model final_GI final_resp final_cardio final_allergy SYM_fat SYM_CA
			=final_dust Age_cat smok/ solution ss3;
  manova h=_ALL_;
run;
quit;
...
%let x_num =2;
%let indata = finaloutput;
%let HealthA=final_resp*final_GI*final_cardio*final_eyes*final_neuro*final_skin*final_allergy*As_COPD*SYM_liver*SYM_uro*SYM_Hor*final_fat*final_CA*SYM_nic*SYM_co*hyper;
%let Exp=final_dust*string_defect;
%let drpvar = Age_catty sex smok;
%macro HealthEff;
%do i=1 %to 16;
%do j=1 %to 2;
proc logistic data=finaloutput desc;
class %scan (&Exp, &j, *)(ref=first) Age_catty (ref=first) sex (ref=first) smok (ref="Non-smoker");
model %scan(&HealthA, &i, *)= %scan (&Exp, &j, *) final_dust string_defect Age_catty sex smok;
run; quit;
%end;
%end;
%mend HealthEff;
...
data job;
set finaloutput
(keep=final_resp final_GI final_cardio final_eyes final_neuro final_skin final_allergy As_COPD SYM_liver SYM_uro SYM_Hor final_fat final_CA SYM_nic SYM_co hyper string_defect final_dust Age_catty sex smok);
run;

%HealthEff(indata=job,
x_num=2,
HealthA=final_resp final_GI final_cardio final_eyes final_neuro final_skin final_allergy As_COPD SYM_liver SYM_uro SYM_Hor final_fat final_CA SYM_nic SYM_co hyper,
Exp=final_dust string_defect,
drpvar=Age_catty sex smok);
Proc 
...
**other codes**
proc logistic data=Ani_odds desc;
*class %scan (&Exp, &j, *)(ref=first)  Age_cat (ref=first) sex (ref=last) smok (ref=first);
*model %scan(&HealthA, &i, *)= %scan (&Exp, &j, *) Age_cat sex smok;
*where sex in ("m", "f");
*run; quit;
*adjusted odds ratio
proc logistic data=finaloutput desc;
class final_dust(ref=first) Age_cat (ref=first) sex (ref=last) smok (ref=first);
model final_GI=final_dust Age_cat sex smok;
where sex in ("m", "f");
run;


where smok not in (0) and (2) and Age_cat not in (2) and (3);
...
*if scan(ID,5,"-")="SVTC" then service_tech=1;
*if service_tech=. then service_tech=0;
*if scan(ID,5,"-")="PRWC" then copy_tech=1;
*if scan(ID,5,"-")="COPC" then copy_tech=1;
*if copy_tech=. then copy_tech=0;
*if scan(ID,5,"-")not in ("OFFC", "JOBC", "ITC", "MEDC", "OTHC", "PENC", "PEDC", "POLC","PRTC", "SVTC", "PRWC", "COPC") then other_officers=1;
*if scan(ID,5,"-")="xxx" then other_officers=0;
*else if other_officers=. then other_officers=0;
*format clerical_officers service_tech copy_tech other_officers YN.;
*run*;*
***%MACRO tran;
%do i= 1 %to &x_num;
%let colnum= %eval(&i+1);
%let vars= %scan(&var_x, &i);
if _name_ = "&vars" then do;
variance = col&colnum;
mean = col1;
end;
%end;
...
%MACRO Logistic (x_num=, indata=, var_y=, var_x=, drpvar=);
PROC logistic data=&indata outest=temp covout;
model 

**trying out a new method to compile the odds ratio**;
%let x_num = 6 ;/*predictor variables*/
%let indata = finaloutput;
%let var_y = final_resp final_GI final_cardio final_eyes final_neuro final_skin final_allergy As_COPD SYM_liver SYM_uro SYM_Hor final_fat final_CA SYM_nic SYM_co hyper;
%let var_x = final_dust Profession_1 string_defect Age_catty sex smok;
%let drpvar = Age_catty sex smok;
&var_y = &var_x;
run; 
